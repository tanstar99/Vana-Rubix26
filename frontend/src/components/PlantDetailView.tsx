...
interface PlantDetailViewProps {
    plantId: string;
    onBack: () => void;
    isOverlay?: boolean; // To adjust styling if needed when used as overlay
}
...
export default function PlantDetailView({ plantId, onBack, isOverlay = false }: PlantDetailViewProps) {
    ...
    if (loading) {
        return (
            <div className={`flex items-center justify-center ${isOverlay ? 'h-full w-full' : 'min-h-screen'} bg-gradient-to-br from-emerald-950 via-green-900 to-emerald-950`}>
    ...
    // If overlay, we add specific overlay classes (like high z-index and absolute positioning) handled by parent,
    // but we might want to ensure background coverage here or make it scrollable.
    const containerClasses = isOverlay
        ? "absolute inset-0 z-50 overflow-y-auto bg-black/90 backdrop-blur-md"
        : "min-h-screen bg-gradient-to-br from-emerald-950 via-green-900 to-emerald-950";

    return (
        <div className={`${containerClasses} pt-24 pb-12 px-4`}>
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex gap-2 mb-6">
                    <button
                        ...
                        className={`px-6 py-3 rounded-xl font-semibold transition-all ${activeTab === 'info'
                    ...
                    <button
                        ...
                        className={`px-6 py-3 rounded-xl font-semibold transition-all ${activeTab === '3d'
                                ? 'bg-emerald-500 text-white'
                                : 'bg-white/10 text-white/80 hover:bg-white/20'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-6 py-3 rounded-xl font-semibold transition-all ${activeTab === 'media'
                                ? 'bg-emerald-500 text-white'
                                : 'bg-white/10 text-white/80 hover:bg-white/20'
                            }`}
                    >
